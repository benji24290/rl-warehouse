    ...
    def plot_state_info_from_episode(self, episode, title=""):
        ...
        for art in range(len(self.state_info_per_episode[episode][0])):
            try:
                ...
                plt.plot(
                    np.add(storage, 12).tolist(), label='Lager Artikel '+str(art+1))
                plt.plot(
                    np.add(requests, 8).tolist(), label='Aufträge Artikel '+str(art+1))
                plt.plot(
                    np.add(arrivals, 4).tolist(), label='Ankunft Artikel '+str(art+1))
                plt.plot(
                    orders, label='Bestellungen Artikel '+str(art+1))
            ...
            plt.title(title+'Artikel '+str(art+1))
            plt.legend()
            plt.show()
