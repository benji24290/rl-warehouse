...
if __name__ == '__main__':
    ...
    if(compare_policies):
        ...
        results_q_learning_policy = run_policy_test_agent(
            env=q_env, num_episodes=episodes, Q=q_env.rewards.import_q("q-learning.csv"), random_seed=random_seed)
        sarsa_env = WarehouseEnv(config)
        results_sarsa_policy = run_policy_test_agent(
            env=sarsa_env, num_episodes=episodes, Q=sarsa_env.rewards.import_q("sarsa.csv"), random_seed=random_seed)

        rew_h_v4 = heuristic(config, count=episodes, version='v4')
